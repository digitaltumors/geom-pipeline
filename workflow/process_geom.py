...
RENAME_FIELDS = {'totalconfs':'Total Conformations',
...
def fetch_molecule_names(smiles:str):
    ...
    try:
        ...
        inchi_key = None if search_results[0].cid is None else search_results[0].inchikey
    except: 
    ...
    for subset_name,file_name in zip(subset_names,file_names):
        ...
        for mol_smiles in (pbar:=tqdm.tqdm(molecule_info.keys())):
            ...
            if os.path.isfile(pickle_file): # CAN DROP FOR FULL
                ...
                valid_idx = [i for i, conf in enumerate(conformations) if 'boltzmannweight' in conf]

                whims = np.array([CalcWHIM(conformations[i]['rd_mol']) for i in valid_idx])
                weights = np.array([conformations[i]['boltzmannweight'] for i in valid_idx])
